...
class Data_Formation(Dataset):
    """
    Класс, загружающий данные для обучения для последующей передачи в класс
    DataLoader. Стандартное требование для этого класса - реализация двух
    "магических" методов __getitem__ для возможности обращения к элементам
    данных как к элементам массива, и __len__ для возможности вычисления
    размера датасета. Данный класс реализует две возможности загрузки данных -
    загрузка в память только необходимого батча данных для экономии памяти
    и возможности работать с большими объёмами данных, и загрузка всего набора
    данных сразу для более быстрого обучения сети, однако, применима только
    для небольших набороб данных.
    ...
    train: bool, optional
        Флаг, указывающий какой надор данных загружать - тренировочный или
        тестовый.
